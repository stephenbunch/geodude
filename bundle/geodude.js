(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.Geodude = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(require,module,exports){
"use strict";
...
GeographicLib.Math.AngDiff = function (x, y) {
  ...
  xpp -= x;
  var t = xpp - yp;
  if (d - 180 + t > 0) d -= 360;else if (d + 180 + t <= 0) d += 360;
  return d + t;
};
...
(function () {
  ...
  g.Geodesic = function (a, f) {
    ...
    this._c2 = (m.sq(this._a) + m.sq(this._b) * (this._e2 === 0 ? 1 : (this._e2 > 0 ? m.atanh(Math.sqrt(this._e2)) : Math.atan(Math.sqrt(-this._e2))) / Math.sqrt(Math.abs(this._e2)))) / 2;
    this._etol2 = 0.1 * g.tol2_ / Math.sqrt(Math.max(0.001, Math.abs(this._f)) * Math.min(1.0, 1 - this._f / 2) / 2);
    if (!(isFinite(this._a) && this._a > 0)) throw new Error("Major radius is not positive");
  ...
  g.Geodesic.prototype.Lengths = function (eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, scalep, C1a, C2a) {
    var vals = {};
    g.C1f(eps, C1a);
    g.C2f(eps, C2a);
    var A1m1 = g.A1m1f(eps),
        AB1 = (1 + A1m1) * (g.SinCosSeries(true, ssig2, csig2, C1a, g.nC1_) - g.SinCosSeries(true, ssig1, csig1, C1a, g.nC1_)),
        A2m1 = g.A2m1f(eps),
        AB2 = (1 + A2m1) * (g.SinCosSeries(true, ssig2, csig2, C2a, g.nC2_) - g.SinCosSeries(true, ssig1, csig1, C2a, g.nC2_));
    vals.m0 = A1m1 - A2m1;
  ...
  g.Geodesic.prototype.InverseStart = function (sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, C1a, C2a) {
    var vals = {},
    ...
    if (shortline && ssig12 < this._etol2) {
      ...
      if (this._f >= 0) {
        ...
        var cbet12a = cbet2 * cbet1 - sbet2 * sbet1,
            bet12a = Math.atan2(sbet12a, cbet12a);
        var m12b, m0;
        var nvals = this.Lengths(this._n, Math.PI + bet12a, sbet1, -cbet1, dn1, sbet2, cbet2, dn2, cbet1, cbet2, false, C1a, C2a);
        m12b = nvals.m12b;m0 = nvals.m0;
        x = -1 + m12b / (cbet1 * cbet2 * m0 * Math.PI);
        betscale = x < -0.01 ? sbet12a / x : -this._f * m.sq(cbet1) * Math.PI;
      ...
      if (y > -g.tol1_ && x > -1 - g.xthresh_) {
        ...
        var k = g.Astroid(x, y);
        var omg12a = lamscale * (this._f >= 0 ? -x * k / (1 + k) : -y * (1 + k) / k);
  ...
  g.Geodesic.prototype.Lambda12 = function (sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1, diffp, C1a, C2a, C3a) {
    var vals = {};
    ...
    vals.sig12 = Math.atan2(Math.max(vals.csig1 * vals.ssig2 - vals.ssig1 * vals.csig2, 0), vals.csig1 * vals.csig2 + vals.ssig1 * vals.ssig2);
    omg12 = Math.atan2(Math.max(comg1 * somg2 - somg1 * comg2, 0), comg1 * comg2 + somg1 * somg2);
    var B312, h0;
    var k2 = m.sq(calp0) * this._ep2;
    vals.eps = k2 / (2 * (1 + Math.sqrt(1 + k2)) + k2);
    this.C3f(vals.eps, C3a);
    B312 = g.SinCosSeries(true, vals.ssig2, vals.csig2, C3a, g.nC3_ - 1) - g.SinCosSeries(true, vals.ssig1, vals.csig1, C3a, g.nC3_ - 1);
    ...
    if (diffp) {
      if (vals.calp2 === 0) vals.dlam12 = -2 * this._f1 * dn1 / sbet1;else {
        var nvals = this.Lengths(vals.eps, vals.sig12, vals.ssig1, vals.csig1, dn1, vals.ssig2, vals.csig2, dn2, cbet1, cbet2, false, C1a, C2a);
  ...
  g.Geodesic.prototype.GenInverse = function (lat1, lon1, lat2, lon2, outmask) {
    ...
    if (meridian) {
      ...
      sig12 = Math.atan2(Math.max(csig1 * ssig2 - ssig1 * csig2, 0), csig1 * csig2 + ssig1 * ssig2);
      nvals = this.Lengths(this._n, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, (outmask & g.GEODESICSCALE) !== 0, C1a, C2a);
      s12x = nvals.s12b;
      m12x = nvals.m12b;
    ...
    var omg12;
    if (!meridian && sbet1 === 0 && (this._f <= 0 || lam12 <= Math.PI - this._f * Math.PI)) {
      ...
      nvals = this.InverseStart(sbet1, cbet1, dn1, sbet2, cbet2, dn2, lam12, C1a, C2a);
      sig12 = nvals.sig12;
      salp1 = nvals.salp1;
      calp1 = nvals.calp1;
      if (sig12 >= 0) {
        ...
        for (var tripn = false, tripb = false; numit < g.maxit2_; ++numit) {
          var dv;
          nvals = this.Lambda12(sbet1, cbet1, dn1, sbet2, cbet2, dn2, salp1, calp1, numit < g.maxit1_, C1a, C2a, C3a);
          ...
          omg12 = nvals.domg12;
          dv = nvals.dlam12;
          if (tripb || !(Math.abs(v) >= (tripn ? 8 : 2) * g.tol0_)) break;
          if (v > 0 && (numit < g.maxit1_ || calp1 / salp1 > calp1b / salp1b)) {
            salp1b = salp1;calp1b = calp1;
          } else if (v < 0 && (numit < g.maxit1_ || calp1 / salp1 < calp1a / salp1a)) {
            salp1a = salp1;calp1a = calp1;
          }
          if (numit < g.maxit1_ && dv > 0) {
            var dalp1 = -v / dv;
            var sdalp1 = Math.sin(dalp1),
        ...
        nvals = this.Lengths(eps, sig12, ssig1, csig1, dn1, ssig2, csig2, dn2, cbet1, cbet2, (outmask & g.GEODESICSCALE) !== 0, C1a, C2a);
        s12x = nvals.s12b;
        m12x = nvals.m12b;
        if ((outmask & g.GEODESICSCALE) !== 0) {
          vals.M12 = nvals.M12;
    ...
    if (outmask & g.AREA) {
      ...
      if (calp0 !== 0 && salp0 !== 0) {
        ...
        this.C4f(eps, C4a);
        var B41 = g.SinCosSeries(false, ssig1, csig1, C4a, g.nC4_),
            B42 = g.SinCosSeries(false, ssig2, csig2, C4a, g.nC4_);
        vals.S12 = A4 * (B42 - B41);
      } else vals.S12 = 0;
      if (!meridian && omg12 < 0.75 * Math.PI && sbet2 - sbet1 < 1.75) {
        var somg12 = Math.sin(omg12),
  ...
  g.Geodesic.prototype.GenDirect = function (lat1, lon1, azi1, arcmode, s12_a12, outmask) {
    var line = new l.GeodesicLine(this, lat1, lon1, azi1, outmask | (arcmode ? g.NONE : g.DISTANCE_IN));
    return line.GenPosition(arcmode, s12_a12, outmask);
  };
  g.WGS84 = new g.Geodesic(GeographicLib.Constants.WGS84.a, GeographicLib.Constants.WGS84.f);
})();
// GeodesicLine.js
(function () {
  var g = GeographicLib.Geodesic;
  ...
  l.GeodesicLine = function (geod, lat1, lon1, azi1, caps) {
    ...
    this._csig1 = this._comg1 = sbet1 !== 0 || this._calp1 !== 0 ? cbet1 * this._calp1 : 1;
    ...
    if (this._caps & g.CAP_C1) {
      ...
      g.C1f(eps, this._C1a);
      this._B11 = g.SinCosSeries(true, this._ssig1, this._csig1, this._C1a, g.nC1_);
    ...
    if (this._caps & g.CAP_C2) {
      ...
      g.C2f(eps, this._C2a);
      this._B21 = g.SinCosSeries(true, this._ssig1, this._csig1, this._C2a, g.nC2_);
    }
    if (this._caps & g.CAP_C3) {
      this._C3a = new Array(g.nC3_);
      geod.C3f(eps, this._C3a);
      this._A3c = -this._f * this._salp0 * geod.A3f(eps);
      this._B31 = g.SinCosSeries(true, this._ssig1, this._csig1, this._C3a, g.nC3_ - 1);
    }
    if (this._caps & g.CAP_C4) {
      this._C4a = new Array(g.nC4_);
      geod.C4f(eps, this._C4a);
      this._A4 = m.sq(this._a) * this._calp0 * this._salp0 * geod._e2;
      this._B41 = g.SinCosSeries(false, this._ssig1, this._csig1, this._C4a, g.nC4_);
    }
  };
  l.GeodesicLine.prototype.GenPosition = function (arcmode, s12_a12, outmask) {
    ...
    if (arcmode) {
      ...
      var tau12 = s12_a12 / (this._b * (1 + this._A1m1)),
          s = Math.sin(tau12),
          c = Math.cos(tau12);
      B12 = -g.SinCosSeries(true, this._stau1 * c + this._ctau1 * s, this._ctau1 * c - this._stau1 * s, this._C1pa, g.nC1p_);
      ...
      if (Math.abs(this._f) > 0.01) {
        ...
        var serr = (1 + this._A1m1) * (sig12 + (B12 - this._B11)) - s12_a12 / this._b;
    ...
    if (outmask & (g.DISTANCE | g.REDUCEDLENGTH | g.GEODESICSCALE)) {
      if (arcmode || Math.abs(this._f) > 0.01) B12 = g.SinCosSeries(true, ssig2, csig2, this._C1a, g.nC1_);
    ...
    if (outmask & g.DISTANCE) vals.s12 = arcmode ? this._b * ((1 + this._A1m1) * sig12 + AB1) : s12_a12;
    if (outmask & g.LONGITUDE) {
      somg2 = this._salp0 * ssig2;comg2 = csig2;
      omg12 = outmask & g.LONG_NOWRAP ? sig12 - (Math.atan2(ssig2, csig2) - Math.atan2(this._ssig1, this._csig1)) + (Math.atan2(somg2, comg2) - Math.atan2(this._somg1, this._comg1)) : Math.atan2(somg2 * this._comg1 - comg2 * this._somg1, comg2 * this._comg1 + somg2 * this._somg1);
      lam12 = omg12 + this._A3c * (sig12 + (g.SinCosSeries(true, ssig2, csig2, this._C3a, g.nC3_ - 1) - this._B31));
      lon12 = lam12 / m.degree;
      lon12 = m.AngNormalize2(lon12);
      vals.lon2 = outmask & g.LONG_NOWRAP ? this._lon1 + lon12 : m.AngNormalize(m.AngNormalize(this._lon1) + m.AngNormalize2(lon12));
    }
    if (outmask & g.LATITUDE) vals.lat2 = Math.atan2(sbet2, this._f1 * cbet2) / m.degree;
    if (outmask & g.AZIMUTH) vals.azi2 = 0 - Math.atan2(-salp2, calp2) / m.degree;
    if (outmask & (g.REDUCEDLENGTH | g.GEODESICSCALE)) {
      var B22 = g.SinCosSeries(true, ssig2, csig2, this._C2a, g.nC2_),
      ...
      if (outmask & g.GEODESICSCALE) {
        var t = this._k2 * (ssig2 - this._ssig1) * (ssig2 + this._ssig1) / (this._dn1 + dn2);
        vals.M12 = csig12 + (t * ssig2 - csig2 * J12) * this._ssig1 / this._dn1;
        vals.M21 = csig12 - (t * this._ssig1 - this._csig1 * J12) * ssig2 / dn2;
      }
    }
    if (outmask & g.AREA) {
      var B42 = g.SinCosSeries(false, ssig2, csig2, this._C4a, g.nC4_);
      var salp12, calp12;
...
(function () {
  ...
  p.transit = function (lon1, lon2) {
    ...
    var cross = lon1 < 0 && lon2 >= 0 && lon12 > 0 ? 1 : lon2 < 0 && lon1 >= 0 && lon12 < 0 ? -1 : 0;
    return cross;
  };
  ...
  p.PolygonArea = function (earth, polyline) {
    ...
    this._mask = g.LATITUDE | g.LONGITUDE | g.DISTANCE | (this._polyline ? g.NONE : g.AREA | g.LONG_NOWRAP);
    if (!this._polyline) this._areasum = new a.Accumulator(0);
  ...
  p.PolygonArea.prototype.Clear = function () {
    this._num = 0;
    this._crossings = 0;
    if (!this._polyline) this._areasum.Set(0);
  ...
  p.PolygonArea.prototype.Compute = function (reverse, sign) {
    ...
    var t = this._earth.Inverse(this._lat1, this._lon1, this._lat0, this._lon0, this._mask);
    ...
    if (crossings & 1) tempsum.Add((tempsum.Sum() < 0 ? 1 : -1) * this._area0 / 2);
    if (!reverse) tempsum.Negate();
    if (sign) {
      if (tempsum.Sum() > this._area0 / 2) tempsum.Add(-this._area0);else if (tempsum.Sum() <= -this._area0 / 2) tempsum.Add(+this._area0);
    } else {
      if (tempsum.Sum() >= this._area0) tempsum.Add(-this._area0);else if (tempsum < 0) tempsum.Add(-this._area0);
    }
  ...
  p.PolygonArea.prototype.TestPoint = function (lat, lon, reverse, sign) {
    ...
    for (var i = 0; i < (this._polyline ? 1 : 2); ++i) {
      t = this._earth.Inverse(i === 0 ? this._lat1 : lat, i === 0 ? this._lon1 : lon, i !== 0 ? this._lat0 : lat, i !== 0 ? this._lon0 : lon, this._mask);
      vals.perimeter += t.s12;
      if (!this._polyline) {
        tempsum += t.S12;
        crossings += p.transit(i === 0 ? this._lon1 : lon, i !== 0 ? this._lon0 : lon);
      }
    }
    if (this._polyline) return vals;
    if (crossings & 1) tempsum += (tempsum < 0 ? 1 : -1) * this._area0 / 2;
    if (!reverse) tempsum *= -1;
    if (sign) {
      if (tempsum > this._area0 / 2) tempsum -= this._area0;else if (tempsum <= -this._area0 / 2) tempsum += this._area0;
    } else {
      if (tempsum >= this._area0) tempsum -= this._area0;else if (tempsum < 0) tempsum += this._area0;
    }
    vals.area = tempsum;
    return vals;
  };
...
(function () {
  ...
  d.Decode = function (dms) {
    ...
    do {
      ...
      if (end > beg && (k = d.lookup(d.hemispheres_, dmsa.charAt(end - 1))) >= 0) {
        if (k >= 0) {
          if (ind1 !== d.NONE) {
            if (dmsa.charAt(beg - 1).toUpperCase() === dmsa.charAt(end - 1).toUpperCase()) errormsg = "Repeated hemisphere indicators " + dmsa.charAt(beg - 1) + " in " + dmsa.substr(beg - 1, end - beg + 1);else errormsg = "Contradictory hemisphere indicators " + dmsa.charAt(beg - 1) + " and " + dmsa.charAt(end - 1) + " in " + dmsa.substr(beg - 1, end - beg + 1);
            break;
          }
          ind1 = k & 2 ? d.LONGITUDE : d.LATITUDE;
          sign = k & 1 ? 1 : -1;
      ...
      while (p < end) {
        ...
        if ((k = d.lookup(d.digits_, x)) >= 0) {
          ...
          if (k >= 3) {
            if (p === end) {
              errormsg = "Illegal for : to appear at the end of " + dmsa.substr(beg, end - beg);
              break;
            }
            k = npiece;
          }
          if (k === npiece - 1) {
            errormsg = "Repeated " + d.components_[k] + " component in " + dmsa.substr(beg, end - beg);
            break;
          } else if (k < npiece) {
            errormsg = d.components_[k] + " component follows " + d.components_[npiece - 1] + " component in " + dmsa.substr(beg, end - beg);
            break;
          }
          if (ncurrent === 0) {
            errormsg = "Missing numbers in " + d.components_[k] + " component of " + dmsa.substr(beg, end - beg);
            break;
          }
          if (digcount > 1) {
    ...
    if (vals.val === 0) throw new Error(errormsg);else vals.ind = d.NONE;
    return vals;
  };
  d.NumMatch = function (s) {
    ...
    var p1 = t.length - 1;
    if (p1 + 1 < p0 + 3) return 0;
    t = t.substr(p0, p1 + 1 - p0);
    if (t === "NAN" || t === "1.#QNAN" || t === "1.#SNAN" || t === "1.#IND" || t === "1.#R") return sign * Number.NaN;else if (t === "INF" || t === "1.#INF") return sign * Number.POSITIVE_INFINITY;
    return 0;
  };
  d.DecodeLatLon = function (stra, strb, swaplatlong) {
    ...
    } else if (ia === d.NONE) ia = d.LATITUDE + d.LONGITUDE - ib;else if (ib === d.NONE) ib = d.LATITUDE + d.LONGITUDE - ia;
    if (ia === ib) throw new Error("Both " + stra + " and " + strb + " interpreted as " + (ia === d.LATITUDE ? "latitudes" : "longitudes"));
    var lat = ia === d.LATITUDE ? a : b,
        lon = ia === d.LATITUDE ? b : a;
    if (Math.abs(lat) > 90) throw new Error("Latitude " + lat + "d not in [-90d, 90d]");
    if (lon < -540 || lon >= 540) throw new Error("Latitude " + lon + "d not in [-540d, 540d)");
  ...
  d.DecodeAngle = function (angstr) {
    ...
    var ang = vals.val,
        ind = vals.ind;
    if (ind !== d.NONE) throw new Error("Arc angle " + angstr + " includes a hemisphere, N/E/W/S");
    return ang;
  };
  d.DecodeAzimuth = function (azistr) {
    ...
    var azi = vals.val,
        ind = vals.ind;
    if (ind === d.LATITUDE) throw new Error("Azimuth " + azistr + " has a latitude hemisphere, N/S");
    if (azi < -540 || azi >= 540) throw new Error("Azimuth " + azistr + " not in range [-540d, 540d)");
    azi = m.AngNormalize(azi);
    return azi;
  };
  d.Encode = function (angle, trailing, prec, ind) {
    if (!ind) ind = d.NONE;
    if (!isFinite(angle)) return angle < 0 ? String("-inf") : angle > 0 ? String("inf") : String("nan");
    ...
    for (i = 0; i < trailing; ++i) {
      ...
      scale *= 10;
    }if (ind === d.AZIMUTH) angle -= Math.floor(angle / 360) * 360;
    var sign = angle < 0 ? -1 : 1;
    angle *= sign;
    var idegree = Math.floor(angle),
    ...
    pieces[0] += idegree;
    var s = new String("");
    if (ind === d.NONE && sign < 0) s += '-';
    switch (trailing) {
      case d.DEGREE:
        s += d.zerofill(pieces[0].toFixed(prec), ind === d.NONE ? 0 : 1 + Math.min(ind, 2) + prec + (prec ? 1 : 0)) + d.dmsindicatorsu_.charAt(0);
        break;
      default:
        s += d.zerofill(pieces[0].toFixed(0), ind === d.NONE ? 0 : 1 + Math.min(ind, 2)) + d.dmsindicatorsu_.charAt(0);
        switch (trailing) {
          case d.MINUTE:
            s += d.zerofill(pieces[1].toFixed(prec), 2 + prec + (prec ? 1 : 0)) + d.dmsindicatorsu_.charAt(1);
            break;
          case d.SECOND:
            s += d.zerofill(pieces[1].toFixed(0), 2) + d.dmsindicatorsu_.charAt(1);
            s += d.zerofill(pieces[2].toFixed(prec), 2 + prec + (prec ? 1 : 0)) + d.dmsindicatorsu_.charAt(2);
            break;
          default:
            break;
        }
    }
    if (ind !== d.NONE && ind !== d.AZIMUTH) s += d.hemispheres_.charAt((ind === d.LATITUDE ? 0 : 2) + (sign < 0 ? 0 : 1));
    return s;
  };
})();
// Interface.js
(function () {
  ...
  g.Geodesic.CheckPosition = function (lat, lon) {
    if (!(Math.abs(lat) <= 90)) throw new Error("latitude " + lat + " not in [-90, 90]");
    if (!(lon >= -540 && lon < 540)) throw new Error("longitude " + lon + " not in [-540, 540)");
    return m.AngNormalize(lon);
  };
  g.Geodesic.CheckAzimuth = function (azi) {
    if (!(azi >= -540 && azi < 540)) throw new Error("longitude " + azi + " not in [-540, 540)");
    return m.AngNormalize(azi);
  };
  g.Geodesic.CheckDistance = function (s) {
    if (!isFinite(s)) throw new Error("distance " + s + " not a finite number");
  };
  ...
  g.Geodesic.prototype.InversePath = function (lat1, lon1, lat2, lon2, ds12, maxk) {
    var t = this.Inverse(lat1, lon1, lat2, lon2);
    if (!maxk) maxk = 20;
    if (!(ds12 > 0)) throw new Error("ds12 must be a positive number");
    var k = Math.max(1, Math.min(maxk, Math.ceil(t.s12 / ds12))),
        points = new Array(k + 1);
    points[0] = { lat: t.lat1, lon: t.lon1, azi: t.azi1 };
    points[k] = { lat: t.lat2, lon: t.lon2, azi: t.azi2 };
    if (k > 1) {
      var line = new l.GeodesicLine(this, t.lat1, t.lon1, t.azi1, g.LATITUDE | g.LONGITUDE | g.AZIMUTH),
          da12 = t.a12 / k;
      var vals;
      for (var i = 1; i < k; ++i) {
        vals = line.GenPosition(true, i * da12, g.LATITUDE | g.LONGITUDE | g.AZIMUTH);
        points[i] = { lat: vals.lat2, lon: vals.lon2, azi: vals.azi2 };
      }
    }
    return points;
  };
  g.Geodesic.prototype.DirectPath = function (lat1, lon1, azi1, s12, ds12, maxk) {
    var t = this.Direct(lat1, lon1, azi1, s12);
    if (!maxk) maxk = 20;
    if (!(ds12 > 0)) throw new Error("ds12 must be a positive number");
    var k = Math.max(1, Math.min(maxk, Math.ceil(Math.abs(t.s12) / ds12))),
        points = new Array(k + 1);
    points[0] = { lat: t.lat1, lon: t.lon1, azi: t.azi1 };
    points[k] = { lat: t.lat2, lon: t.lon2, azi: t.azi2 };
    if (k > 1) {
  ...
  g.Geodesic.prototype.Circle = function (lat1, lon1, azi1, s12, k) {
    if (!(Math.abs(lat1) <= 90)) throw new Error("lat1 must be in [-90, 90]");
    if (!(lon1 >= -540 && lon1 < 540)) throw new Error("lon1 must be in [-540, 540)");
    if (!(azi1 >= -540 && azi1 < 540)) throw new Error("azi1 must be in [-540, 540)");
    if (!isFinite(s12)) throw new Error("s12 must be a finite number");
    lon1 = m.AngNormalize(lon1);
    ...
    for (var i = 0; i <= k; ++i) {
      ...
      vals = this.GenDirect(lat1, lon1, azi1a, false, s12, g.LATITUDE | g.LONGITUDE);
  ...
  g.Geodesic.prototype.Envelope = function (lat1, lon1, k, ord) {
    ...
    for (var i = 0; i <= k; ++i) {
      ...
      line = new l.GeodesicLine(this, lat1, lon1, azi1, g.LATITUDE | g.LONGITUDE | g.DISTANCE_IN | g.DISTANCE | g.REDUCEDLENGTH | g.GEODESICSCALE);
      vals = line.GenPosition(true, 180 * ord, g.DISTANCE | g.REDUCEDLENGTH | g.GEODESICSCALE);
      j = 0;
      while (true) {
        s12 = vals.s12 - vals.m12 / vals.M21;
        if (Math.abs(vals.m12) < line._a * g.tol2_ * 0.1 || ++j > 10) break;
        vals = line.GenPosition(false, s12, g.DISTANCE | g.REDUCEDLENGTH | g.GEODESICSCALE);
      }
...
var _GeographicLib = require('./GeographicLib');
...
function distanceToPoint(origin, destination) {
  return _GeographicLib2.default.Geodesic.WGS84.GenInverse(origin[1], origin[0], destination[1], destination[0], _GeographicLib2.default.Geodesic.DISTANCE).s12;
}
...
function destinationPoint(origin, azi, distance) {
  var result = _GeographicLib2.default.Geodesic.WGS84.GenDirect(origin[1], origin[0], azi, false, distance, _GeographicLib2.default.Geodesic.LATITUDE | _GeographicLib2.default.Geodesic.LONGITUDE);
  return [result.lon2, result.lat2];
}

function initialBearing(origin, destination) {
  var result = _GeographicLib2.default.Geodesic.WGS84.GenInverse(origin[1], origin[0], destination[1], destination[0], _GeographicLib2.default.Geodesic.AZIMUTH);
  return result.azi2;
}
...
function gnForward(center, coord) {
  var result = _GeographicLib2.default.Geodesic.WGS84.GenInverse(center[1], center[0], coord[1], coord[0], _GeographicLib2.default.Geodesic.AZIMUTH | _GeographicLib2.default.Geodesic.REDUCEDLENGTH | _GeographicLib2.default.Geodesic.GEODESICSCALE);
  var x, y;
  if (result.M12 <= 0) {
    x = y = NaN;
  } else {
    var rho = result.m12 / result.M12;
    var azi0 = result.azi1 * _GeographicLib2.default.Math.degree;
    x = rho * Math.sin(azi0);
    y = rho * Math.cos(azi0);
  }
  return [x, y];
...
function gnReverse(center, point) {
  var _a = _GeographicLib2.default.Geodesic.WGS84._a;
  var eps_ = 0.01 * Math.sqrt(Number.EPSILON);
  var x = point[0];
  var y = point[1];

  var azi0 = Math.atan2(x, y) / _GeographicLib2.default.Math.degree;
  ...
  var line = new _GeographicLib2.default.GeodesicLine.GeodesicLine(_GeographicLib2.default.Geodesic.WGS84, center[1], center[0], azi0, _GeographicLib2.default.Geodesic.LATITUDE | _GeographicLib2.default.Geodesic.LONGITUDE | _GeographicLib2.default.Geodesic.AZIMUTH | _GeographicLib2.default.Geodesic.DISTANCE_IN | _GeographicLib2.default.Geodesic.REDUCEDLENGTH | _GeographicLib2.default.Geodesic.GEODESICSCALE);
  var count = 10;
  var trip = 0;
  var pos;
  while (count--) {
    pos = line.GenPosition(false, s, _GeographicLib2.default.Geodesic.LATITUDE | _GeographicLib2.default.Geodesic.LONGITUDE | _GeographicLib2.default.Geodesic.AZIMUTH | _GeographicLib2.default.Geodesic.REDUCEDLENGTH | _GeographicLib2.default.Geodesic.GEODESICSCALE);
    if (trip) {
      break;
    }
    // If little, solve rho(s) = rho with drho(s)/ds = 1/M^2
    // else solve 1/rho(s) = 1/rho with d(1/rho(s))/ds = -1/m^2
    var ds = little ? (pos.m12 / pos.M12 - rho) * pos.M12 * pos.M12 : (rho - pos.M12 / pos.m12) * pos.m12 * pos.m12;
    s -= ds;
...
//# sourceMappingURL=geodude.js.map?78c07e157098aaca1041c2e14a0ffecdf4776533
